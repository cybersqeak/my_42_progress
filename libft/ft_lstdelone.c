...
int main(void)
{
  ...
  //  printf("the content_address of list is (must be exist) : %p\n", list->content);
   ...
   // printf("the content_address of list is (must not  be exist) : %p\n", list->content); // well it turn out to be existed , but this is called dangling pointer...  a pointer that points to a memory location that has been deleted, freed, or has gone out of scope
    ...
    //list->content = NULL; 
    //printf("the content_address of list is (must be nonexist) : %p", list->content);
    return (0);
}
*/
